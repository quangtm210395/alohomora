...
export class Config {
  ...
  /**
   * the policy enforcer writing in json format
   */
  policyEnforcer: any;
   ...
   * @constructor
   */
  constructor(config?: AlohomoraConfig | String) {
    const cfg = config ? config : path.join(process.cwd(), 'keycloak.json');
   ...
   *
   * @param {String} cfgPath Path to a `keycloak.json` configuration.
   */
  loadConfiguration(cfgPath: string) {
    const json = readFileSync(cfgPath);
  ...
  configure(config: any) {
    ...
    const plainKey: string = resolveValue(config['realm-public-key'] || config.realmPublicKey);
    ...
    this.jsonEnforcerEnabled = resolveValue(config['json-enforcer-enabled'] || config.jsonEnforcerEnabled || false);
    this.policyEnforcer = config['policy-enforcer'] || config.policyEnforcer || {};
  }
}
